...
public class MssqlRecensioniDAO implements RecensioniDAO {
	...
	static final String create = "INSERT INTO " + TABLE_NAME + "(idRecensore,idRecensito,voto, titolo, corpo)"
			+" VALUES(?,?,?,?,?)";
	static final String delete = "DELETE FROM " + TABLE_NAME + " WHERE idRecensore=? AND idRecensito=?";
	static final String update = "UPDATE"+TABLE_NAME +"SET voto=?,corpo=?,titolo=?,risposta=?"+
	"WHERE id=?";
	...
	public boolean update(Recensione r) {
		...
		try {
			/**
			update = "UPDATE"+TABLE_NAME +"SET voto=?,corpo=?,titolo=?,risposta=?,"+
	        "WHERE id=?";*/
	...
	public List<Recensione> readRecensioniUtente(Utente u) {
		...
		String query = "SELECT idRecensore, idRecensito, voto, corpo, titolo FROM Recensione WHERE idRecensito=?";
		try {
	...
	public void setConn(Connection dbConnection) {
		if(this.conn != null)
			try {
				this.conn.close();
			} catch (SQLException e) {
